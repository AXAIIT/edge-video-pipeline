...
RUN_ID="${RUN_ID:-$(date +%Y%m%d)_rdk_x5_8gb_pipeline_failure_test}"
PYTHON_BIN="${PYTHON_BIN:-python3}"
APP="${APP:-build/03_video_pipeline_rdk_x5/video_pipeline_app}"
OUTPUT="${OUTPUT:-logs/failures/03_video_pipeline/rdk_x5_8gb/${RUN_ID}_failure_injection.jsonl}"
...
stage: rdk_x5_failure_injection
status: not_verified
...
"${PYTHON_BIN}" projects/03_video_pipeline/scripts/inject_failure_tests.py \
  ...
  --backend-config projects/03_video_pipeline/configs/boards/rdk_x5_8gb.yaml \
  ...
  --missing-artifact-path models/yolo11n/rdk_x5_bpu_split_head/does_not_exist.bin \
