...
def extract_frame_id(record):
    for key in ("file_name", "image_path"):
        value = record.get(key)
        if not value:
            continue
        match = FRAME_NAME_RE.search(str(value))
        if match:
...
def load_decoded_detections(record):
    ...
    with path.open("r", encoding="utf-8-sig") as f:
        payload = json.load(f)
    detections = payload.get("detections") if isinstance(payload, dict) else payload
...
def build_output_row(record, manifest, frame_id, detections):
    ...
    for det in detections:
        bbox_xyxy = det.get("xyxy_original") or det.get("bbox_xyxy") or det.get("xyxy_640")
        if bbox_xyxy is None:
            raise SystemExit(
                f"{record.get('decoded_output_path')}: detection missing xyxy_original/bbox_xyxy/xyxy_640"
            )
        normalized.append(
            {
    ...
    return {
        ...
        "frame_id": frame_id,
        "frame_image_id": record.get("file_name") or record.get("image_path"),
        "output_valid": True,
...
def main():
    ...
    parser.add_argument("--project2-index", required=True)
    ...
    for record in load_jsonl(args.project2_index):
        if record.get("status") != "pass":
            continue
        frame_id = extract_frame_id(record)
        if frame_id is None:
            continue
        if frame_id in groups:
            raise SystemExit(f"duplicate frame_id in project2 index: {frame_id}")
        record["_source_raw_result_path"] = args.project2_index
        groups[frame_id] = record

    missing = [frame_id for frame_id in expected_frame_ids if frame_id not in groups]
    if missing:
        sample = ", ".join(str(x) for x in missing[:10])
        raise SystemExit(f"missing {len(missing)} manifest frames in project2 index: {sample}")

    output_rows = []
    for frame_id in expected_frame_ids:
        record = groups[frame_id]
    ...
    print(f"source_project2_index={args.project2_index}")
